  ...
  <div class="container my-5">
    ...
    <div class="row justify-content-between">
      ...
      <div class="col-2 col-md-4 col-lg-4">
        <RouterLink :to="{ name: 'management-global-document-create'}" class="btn btn-sm btn-primary float-end my-3 ms-auto">Dodaj</RouterLink>
      </div>
    </div>
    <div class="row">
      <div class="col-12">
          ...
          <table class="table table-hover text-center align-middle fs-6">
            ...
            <tbody>
              <tr v-for="documentType in documentTypes" :key="documentType.id">
                <td>
                  <RouterLink :to="{ name: 'management-global-document-view', params: { id: documentType.id } }" class="text-decoration-none">
                    {{ documentType.name }}
                  </RouterLink>
                </td>
                <td>
...
import type { DocumentTypeDTO } from "@/modules/management/api/dtos/document-type/DocumentTypeDTO"

const title = 'Lista rodzajów dokumentów'
...
documentTypes.value = await documentTypeService.getDocumentTypes()
...
const deleteDocumentType = (documentTypeId: number) => {
  try {
    Swal.fire({
      icon: 'question',
      html: 'Czy na pewno chcesz usunąć ten rodzaj dokumentu?',
      ...
      async (result) => {
        if (result.isConfirmed) {
          ...
          Swal.fire({
            ...
            position: 'top-end',
            icon: 'success',
            title: 'Rodzaj dokumentu usunięty',
            timer: 1000,
