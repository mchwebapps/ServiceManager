  ...
  <div class="container">
    <div class="row bg-light border rounded m-0 py-3 px-2">
      ...
      <div class="col-12 mb-3">
        <input type="text" class="form-control" placeholder="Wpisz nazwę rodzaju płatności" v-model="paymentMethod.name" autocomplete="off">
      </div>
    </div>
    <hr>
    <div class="d-flex justify-content-between mb-5">
      <RouterLink :to="{ name: 'management-global-payment-list' }" class="btn btn-secondary">Zamknij</RouterLink>
      <button class="btn btn-primary" @click="createPaymentMethod()" :disabled="submitBtnLock">Dodaj</button>
    </div>
...
const createPaymentMethod = async () => {
  try {
    ...
    await paymentMethodService.createPaymentMethod(paymentMethod.value as PaymentMethodDTO)

    Swal.fire({
      toast: true,
      position: 'top-end',
      icon: 'success',
      title: 'Rodzaj płatności utworzony',
      timer: 1000,
      ...
      paymentMethod.value = new PaymentMethodDTO
    })
  }
  catch (error: any) {
