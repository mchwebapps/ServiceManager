  ...
  <div class="container">
    <div class="row bg-light border rounded m-0 py-3 px-2">
      ...
      <div class="col-12 mb-3">
        <input type="text" class="form-control" placeholder="Wpisz nazwę rodzaju dokumentu" v-model="documentType.name" autocomplete="off">
      </div>
    </div>
    <hr>
    <div class="d-flex justify-content-between mb-5">
      <RouterLink :to="{ name: 'management-global-document-list' }" class="btn btn-secondary">Zamknij</RouterLink>
      <button class="btn btn-primary" @click="createDocumentType()" :disabled="submitBtnLock">Dodaj</button>
    </div>
...
const createDocumentType = async () => {
  try {
    ...
    await documentTypeService.createDocumentType(documentType.value as DocumentTypeDTO)

    Swal.fire({
      toast: true,
      position: 'top-end',
      icon: 'success',
      title: 'Rodzaj dokumentu utworzony',
      timer: 1000,
      ...
      documentType.value = new DocumentTypeDTO
    })
  }
  catch (error: any) {
