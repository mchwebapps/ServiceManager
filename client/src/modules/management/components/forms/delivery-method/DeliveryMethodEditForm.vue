  ...
  <div class="container">
    <div class="row bg-light border rounded m-0 py-3 px-2">
      ...
      <div class="col-12 mb-3">
        <input type="text" class="form-control" placeholder="Wpisz nazwę sposobu dostawy" v-model="deliveryMethodEdit.name" autocomplete="off">
      </div>
    </div>
    <hr>
    <div class="d-flex justify-content-between mb-5">
      <RouterLink :to="{ name: 'management-global-delivery-list' }" class="btn btn-secondary">Zamknij</RouterLink>
      <button class="btn btn-warning" @click="updateDelivery()" :disabled="submitBtnLock">Zapisz</button>
    </div>
...
const props = defineProps({
  deliveryMethod: { type: DeliveryMethodDTO, required: true }
})
const router = useRouter()
const deliveryMethodEdit = ref<DeliveryMethodDTO>({...props.deliveryMethod})
...
const updateDelivery = async () => {
  try {
    ...
    await deliveryMethodService.updateDeliveryMethod(props.deliveryMethod.id, deliveryMethodEdit.value)

    Swal.fire({
      toast: true,
      position: 'top-end',
      icon: 'success',
      title: 'Sposób dostawy zapisany',
      timer: 1000,
      ...
      deliveryMethodEdit.value = new DeliveryMethodDTO
    })
  }
  catch (error: any) {
