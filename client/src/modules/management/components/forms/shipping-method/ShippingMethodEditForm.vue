  ...
  <div class="container">
    <div class="row bg-light border rounded m-0 py-3 px-2">
      ...
      <div class="col-12 mb-3">
        <input type="text" class="form-control" placeholder="Wpisz nazwę sposobu wysyłki" v-model="shippingMethodEdit.name" autocomplete="off">
      </div>
    </div>
    <hr>
    <div class="d-flex justify-content-between mb-5">
      <RouterLink :to="{ name: 'management-global-shipping-list' }" class="btn btn-secondary">Zamknij</RouterLink>
      <button class="btn btn-warning" @click="updateShippingMethod()" :disabled="submitBtnLock">Zapisz</button>
    </div>
...
const props = defineProps({
  shippingMethod: { type: ShippingMethodDTO, required: true }
})
const router = useRouter()
const shippingMethodEdit = ref<ShippingMethodDTO>({...props.shippingMethod})
...
const updateShippingMethod = async () => {
  try {
    ...
    await shippingMethodService.updateShippingMethod(props.shippingMethod.id, shippingMethodEdit.value)

    Swal.fire({
      toast: true,
      position: 'top-end',
      icon: 'success',
      title: 'Sposób wysyłki zapisany',
      timer: 1000,
      ...
      shippingMethodEdit.value = new ShippingMethodDTO
    })
  }
  catch (error: any) {
