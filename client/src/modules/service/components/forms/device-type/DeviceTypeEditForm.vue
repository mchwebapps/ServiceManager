<template>
  <div class="container">
    ...
    <div class="row bg-light border rounded m-0 pt-3 pb-0 px-2">
      ...
      <div class="col-12 mb-3">
        <div class="input-group">
          ...
          <button class="btn btn-success ms-2" @click="addService(selectedService)" :disabled="selectedService < 0">Dodaj</button>
        </div>
        <div v-if="addedServices">
          <hr class="border-secondary border-opacity-75">
          <div class="col-12 mb-3" v-for="service in addedServices" :key="service.id">
            <div class="input-group input-group-sm">
    ...
    <div class="d-flex justify-content-between mb-5">
      ...
      <button class="btn btn-warning" @click="updateDeviceType()" :disabled="submitBtnLock">Zapisz</button>
    </div>
...
deviceType.value = await deviceTypeService.getDeviceType(props.deviceTypeId)
...
const addedParameters = ref<ParameterDTO[]>(deviceType.value.parameters)
const selectedParameter = ref<number>(0)
const addedServices = ref<ServiceDTO[]>(deviceType.value.services)
...
const addParameter = async (parameterId: number) => {
  if (!addedParameters.value.find(ap => ap.id == parameterId) && selectedParameter.value > 0) {
    const parameter = parameters.value.find(p => p.id == parameterId) as ParameterDTO
    addedParameters.value.push(parameter)
  }
...
const addService = async (serviceId: number) => {
  if (!addedServices.value.find(as => as.id == serviceId) && selectedService.value > 0) {
    const service = services.value.find(s => s.id == serviceId) as ServiceDTO
